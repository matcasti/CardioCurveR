...
This function looks to perform the non-linear regression, estimating the seven parameters ($\alpha, \beta, \lambda, \tau, c, \phi, \delta$) of the dual-logistic model based on observed time and RRi data.

It employs R's powerful `optim()` function for numerical optimization. By default, it uses the `"L-BFGS-B"` method, which is a quasi-Newton algorithm capable of handling box constraints (lower and upper bounds) on the parameters. This is important for ensuring physiologically plausible parameter estimates (e.g., constraining $\alpha$ to be positive). See `?estimate_RRi_curve()` for its documentation.
